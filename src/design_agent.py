...
from utils import read_sys_prompt,read_user_prompt
from config import Config

load_dotenv()
class DesignAgent:
    def __init__(self, prompt:dict=None, config=None):
        self.messages = []
...
if __name__ == "__main__":
    # 创建配置实例
    config = Config()
    agent = DesignAgent(read_sys_prompt("prompt/sys_prompt.txt"), config)  
    print(agent.invoke(read_user_prompt("prompt/human_prompt.txt",center_frequency=100,bandwidth=40)))
